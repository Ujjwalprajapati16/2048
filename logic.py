...
def start_game():
    ...
    print("'S' or 's' : moves down")
    print("'A' or 'a' : move left")
    print("'D' or 'd' : moves right")

    # calling a function to add a new 2 in grid after every step
    add_new_2(mat)
    return mat

# function to add a new 2 in grid at any random empty cell
def add_new_2(mat):
    # chhose a random index for row and coloumn 
    r = random.randint(0, 3)
    c = random.randint(0, 3)
    #while loop will break as the random cell chosen will be empty
    while(mat[r] != 0):
        r = random.randint(0, 3)
        c = random.randint(0, 3)
    
    # we will place at 2 that empty random cell
    mat[r] = 2
...
# functions to compress the grid after every step before and after merging
def compress(mat):
    # bbol variable to determine any change happend or not
    changed = False
    ...
    # here we will shift enetries of each cell to its extreme left row by row
    for i in range(4):
        pos = 0
        ...
        for j in range(4):
            if(mat[i][j] != 0):
                #if cell is non empty then we will shift its number previous empty cell in the
...
def move_left(grid):
    ...
    # return new matrix and bool changed telling wheater the grid is same or different
    return new_grid, changed

# for move / swipe right
def move_right(grid):
    ...
    new_grid = reversed(new_grid)
    return new_grid, changed

# if we move / swipe up
def move_up(grid):
    # to move up we just take transpose the matrix
